...
class _TestScreenState extends State<TestScreen> {
  int _selectedOption = -1; // To keep track of the selected option
  int currentPage = 1; // Current page
  int totalPages = 2; // Total number of pages (based on the response)
  final PageController _pageController = PageController();
  late Timer _timer;
  int _remainingTime = 300; // Start from 300 seconds (5 minutes)
  bool _modalShown = false; // Track if the submit modal has been shown
  ...
  final Map<String, dynamic> testData = {
    "chapterIndex": 1,
    "lessonIndex": 1,
    "testName": "Chandhan Test",
    "totalQuestions": 2,
    "questions": [
      {
        "id": 1,
        "questionStatement": "What is the limit of sin(x)/x as x approaches 0?",
        "options": ["0", "1", "Infinity", "Undefined"],
        "questionImageUrl":
            "https://www.sciencefacts.net/wp-content/uploads/2023/03/Viscosity.jpg"
      },
      {
        "id": 2,
        "questionStatement": "What is the value of the limit lim x->0 (1/x)?",
        "options": ["0", "1", "Infinity", "Undefined"],
        "questionImageUrl":
            "https://www.sciencefacts.net/wp-content/uploads/2023/03/Viscosity.jpg"
      }
  ...
  void initState() {
    super.initState();
    totalPages = testData[
        'totalQuestions']; // Set total pages based on the number of questions
    _startTimer();
  }
  ...
  void _startTimer() {
    _timer = Timer.periodic(const Duration(seconds: 1), (timer) {
      if (_remainingTime > 0) {
        ...
        _timer.cancel();
        // Time's up, navigate to result screen
        Navigator.pushReplacement(
          context,
          MaterialPageRoute(
            builder: (context) => const ResultScreen(isTimeout: true),
                        ...
                        child: OutlinedButton(
                          onPressed: () {
                            ...
                            Navigator.pushReplacement(
                              ...
                              MaterialPageRoute(
                                builder: (context) =>
                                    const ResultScreen(isTimeout: false),
                              ),
                            );
                          },
                          style: OutlinedButton.styleFrom(
          ...
          children: [
            ...
            Expanded(
              child: PageView.builder(
                ...
                itemBuilder: (context, index) {
                  var question = testData['questions'][index];
                  return Padding(
                    padding: const EdgeInsets.symmetric(horizontal: 10),
                    child: Column(
                      children: [
                        ...
                        ...List.generate(
                          ...
                          (i) => _buildOptionCard(question['options'][i], i),
                        ),
                      ],
                    ),
                  );
                    ...
                    child: Column(
                      ...
                      children: [
                        SizedBox(
                          width: 240,
                          child: Text(
                            'Ch${testData['chapterIndex']}, L${testData['lessonIndex']}: ${testData['testName']}',
                            style: const TextStyle(
                              fontSize: 18,
                              fontWeight: FontWeight.bold,
                              overflow: TextOverflow.ellipsis,
