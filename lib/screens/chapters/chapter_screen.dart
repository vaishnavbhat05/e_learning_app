...
class _ChapterScreenState extends State<ChapterScreen> {
  ...
  void _fetchData() {
    ...
    // Fetch chapters only once when screen loads or needs refreshing
    if (chapterProvider.chapters.isEmpty) {
      chapterProvider.fetchChapters(widget.subjectId).then((_) {
        if (chapterProvider.chapters.isNotEmpty && mounted) {
          ...
          chapterProvider.fetchLessonsByChapter(
            chapterProvider.chapters[0].id,
          );
  ...
  void _onChapterSelected(int index) {
    ...
    final chapterProvider = Provider.of<ChapterProvider>(context, listen: false);
    chapterProvider.fetchLessonsByChapter(
      chapterProvider.chapters[index].id,
    );
  }
  ...
  Future<void> _refreshContent() async {
    final chapterProvider = Provider.of<ChapterProvider>(context, listen: false);
    await chapterProvider.fetchLessonsByChapter(
      chapterProvider.chapters[selectedChapterIndex!].id,
    );
  }
        ...
        children: List.generate(provider.chapters.length, (index) {
          ...
          return Padding(
            ...
            child: GestureDetector(
              onTap: () {
                _onChapterSelected(index);  // When a chapter is tapped, only update lessons
              },
              child: Container(
                width: 160,
                height: 175,
  ...
  Widget _buildLessonProgressCard(ChapterProvider provider, Lesson lesson) {
    ...
    return GestureDetector(
      onTap: () async {
        ...
        if (isUnlocked && (lesson.lessonId == lesson.lessonId || lesson.currentlyStudyingLesson || allTopicsCompleted)) {
          await Provider.of<ChapterProvider>(context, listen: false).fetchLessonDetails(lesson.chapterId, lesson.lessonId);
          Navigator.push(
            context,
            MaterialPageRoute(
                      ...
                      children: [
                        Column(
                          children: [
                            ...
                            Expanded(child: _buildLessonList(chapterProvider)),
                          ],
                        ), // ALL Tab
                  ...
                  children: [
                    CircleAvatar(
                      ...
                      child: const Icon(Icons.eco,
                          color: Colors.black, size: 32), // Larger icon
                    ),
                          ...
                          RichText(
                            text: TextSpan(
                              children: [
                                ...
                                TextSpan(
                                text: '${topic.heading}', // Display the title below
                                style: const TextStyle(
                                fontWeight: FontWeight.bold,
